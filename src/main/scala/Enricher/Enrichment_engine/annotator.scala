...
object annotator {
  ...
  def get_info(source: String, code: String, raw_value: String,table: String, column: String): List[Map[String, String]] = {
    ...
    if (tmp.nonEmpty) {
      ...
      result :+= Map("source" -> onto, "iri" -> tmp.head(1), "label" -> tmp.head(2), "xref" -> tmp.head(3), "syn" -> tmp.head(4), "parents" -> tmp.head(5), "part_of" -> tmp.head(7),"description"->tmp.head(8),"iri"->tmp.head.last)
      ...
      for (elem <- anc) {
        ...
        result :+= Map("source" -> elem._1, "iri" -> elem._2, "label" -> elem._3, "xref" -> elem._4, "syn" -> elem._5, "parents" -> elem._6, "part_of" -> elem._8,"description"->elem._9,"iri"->elem._10)
      }
