...
object enrichment_engine {
  ...
  def controller(table_name: String = ""): Unit = {
    ...
    for (term_type <- t) {
      val raw_values = db_handler.get_raw_values(table_name,term_type)
      for (raw_value <- raw_values) {
        println(term_type)
        println(raw_value)
        val result_syn = db_handler.get_cv_support_syn_by_value(raw_value.map(_.toLower))
        val result_user_changes = db_handler.get_raw_user_changes(table_name,term_type,raw_value.map(_.toLower))
        ...
        if(result_syn.tid != default_values.int || result_user_changes._1 != "null") {
          ...
          //CHECK IF RAW VALUE EXIST IN USER CHANGES
            ...
            val a = annotator.ols_get_onto_info(source)
