      ...
      if (result_raw.tid != default_values.int) {
        ...
        if (result_syn.ttype == "pref") {
          ...
          if (!db_handler.user_fb_exist(raw_value, suggestion.source, suggestion.code)) {
            db_handler.user_feedback_insert(List(expert_choice_type(default_values.int, default_values.bool, table_name, column_name, Some(result_syn.tid), raw_value, null, Some(suggestion.label), Some(suggestion.source), Some(suggestion.code), Some(suggestion.iri), "LOCAL:PREF", get_timestamp())))
          }
        }
        else {
          ...
          val suggestion = db_handler.get_cv_support_by_tid(result_syn.tid)
          if (!db_handler.user_fb_exist(raw_value, suggestion.source, suggestion.code)) {
            db_handler.user_feedback_insert(List(expert_choice_type(default_values.int, default_values.bool, table_name, column_name, Some(result_syn.tid), raw_value, null, Some(suggestion.label), Some(suggestion.source), Some(suggestion.code), Some(suggestion.iri), "LOCAL:SYN", get_timestamp())))
          }
        }
      }
        ...
        if (result_search.options.nonEmpty) {
          if (result_search.options.length == 1) {
            ...
            for (elem <- result_search.options){
              ...
              db_handler.user_feedback_insert(List(expert_choice_type(default_values.int, default_values.bool, table_name, column_name, null, raw_value, null, Some(label), Some(source), Some(code), Some(Ols_interface.ols_get_iri(source,code)), "ONLINE:UNDECIDED "+result_search.score, get_timestamp())))
            }
          }
        }
        else {
