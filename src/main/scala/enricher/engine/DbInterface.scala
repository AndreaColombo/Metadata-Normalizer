...
object DbInterface {
  ...
  def db_interface(res: List[Map[String, String]], raw_value: String, table_name: String, column_name: String, method: Char, source: String, code: String): Unit = {
    ...
    val mainTidOption = DbHandler.get_tid_option(source, code)
  ...
  def unfold_hyp(cur_tid: Int): Unit = {
    val tid_list = DbHandler.get_tid_parent_distinct(cur_tid)
    var unfolded: List[relationship_unfolded_type] = List()
    for (tid_parent_cur <- tid_list) {
      val distance = 1
      val onto_support_hyp_l = DbHandler.get_onto_hyp(tid_parent_cur)
    ...
    DbHandler.insert_unfolded(unfolded)
  }
  ...
  def unfold_recursive(tid_parent_cur: Int, tid_child_cur: Int, distance: Int, rel_type_cur: String): List[relationship_unfolded_type] = {
    ...
    val onto_support_hyp_l = DbHandler.get_onto_hyp(tid_child_cur)
