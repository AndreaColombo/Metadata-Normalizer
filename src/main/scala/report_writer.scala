...
object report_writer {
  ...
  def write_report(t: String): Unit = {
    ...
    r = """\subsubsection{Best ontologies sets}"""+"\n"
    val onto_sets = db_handler.get_best_ontos_per_term(t)
    var i = 0
    for (os <- onto_sets if i < 3){
      ...
      r +=
        """\begin{itemize}"""+"\n"+
          ...
          """\item Score: """+score.toString+"\n"+
          """\item Suitability: """+suit.toString+"\n"+
          """\item Categories: """+categories.dropRight(1)+"\n"+
          """\end{itemize}"""+"\n"
      r+="\n"
      i+=1
    }

    val f = new File("report2_" + t.replace("_","-") + ".csv")
