...
object RecommenderParser {
  ...
  def parse (s: String, map: Map[String, Seq[String]]=null): List[List[String]] = {
    ...
    for (i <- l_parsed_value.indices){
      ...
      val j2 = Json.parse(Http(url).param("apikey", apikey).param("display_context","false").header("accept", "text/json").option(HttpOptions.connTimeout(10000)).option(HttpOptions.readTimeout(50000)).asString.body)

      val prefLabel = (j2 \ "prefLabel").validate[String].get.map(_.toLower)
